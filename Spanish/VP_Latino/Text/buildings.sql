...
--Barracks
	...
	SET Text = '+15 XP para todas las unidades. Incrementa el suministro de unidades militares en 1.[NEWLINE][NEWLINE]-1 [ICON_HAPPINESS_3] Infelicidad por [ICON_FOOD]/[ICON_PRODUCTION] Angustia.'
...
--Council
	...
	SELECT 'TXT_KEY_BUILDING_GROVE', 'Consejo'
	WHERE EXISTS (SELECT * FROM COMMUNITY WHERE Type='COMMUNITY_CORE_BALANCE_BUILDINGS' AND Value= 1 );

	INSERT INTO Language_es_ES (Tag, Text)
	SELECT 'TXT_KEY_BUILDING_GROVE_STRATEGY', 'El Consejo genera ciencia cada vez que una ciudad crece en población, y está disponible muy temprano. Construye este edificio si deseas una ventaja tecnológica sobre tus rivales, aunque el valor de esta ventaja disminuye a medida que crece la ciudad.'
	WHERE EXISTS (SELECT * FROM COMMUNITY WHERE Type='COMMUNITY_CORE_BALANCE_BUILDINGS' AND Value= 1 );

	INSERT INTO Language_es_ES (Tag, Text)
	SELECT 'TXT_KEY_CIV5_BUILDINGS_GROVE_TEXT', 'Un consejo es un órgano de gobierno en una comunidad organizada. Es más común en las culturas de subsistencia, siendo la pertenencia a un consejo la condición o la cualidad de ser un líder político de una comunidad. Con el tiempo, a medida que las comunidades crecieron, la importancia de los consejos disminuyó a favor de instituciones de gobierno más grandes.'
	WHERE EXISTS (SELECT * FROM COMMUNITY WHERE Type='COMMUNITY_CORE_BALANCE_BUILDINGS' AND Value= 1 );
--Court Chapel (Tradition building)
	INSERT INTO Language_es_ES (Tag, Text)
	SELECT 'TXT_KEY_BUILDING_PALACE_COURT_CHAPEL', 'Capilla de la Corte'
	WHERE EXISTS (SELECT * FROM COMMUNITY WHERE Type='COMMUNITY_CORE_BALANCE_BUILDINGS' AND Value= 1 );

	INSERT INTO Language_es_ES (Tag, Text)
	SELECT 'TXT_KEY_BUILDING_PALACE_COURT_CHAPEL_HELP', 'Añade un Artista especialista en la [ICON_CAPITAL] Capital. Contiene un espacio para [ICON_GREAT_WORK] Grandes Obras de Arte. Solo puede construirse por medio de la política [COLOR_MAGENTA]Soberanía[ENDCOLOR].'
	WHERE EXISTS (SELECT * FROM COMMUNITY WHERE Type='COMMUNITY_CORE_BALANCE_BUILDINGS' AND Value= 1 );

	INSERT INTO Language_es_ES (Tag, Text)
	SELECT 'TXT_KEY_BUILDING_PALACE_COURT_CHAPEL_PEDIA', 'Una capilla es un lugar religioso de compañerismo, oración y adoración que está adjunto a una institución más grande, a menudo no religiosa, o que se considera una extensión de una institución religiosa primaria. Puede ser parte de una estructura o complejo más grande, como una universidad, hospital, palacio, prisión, funeraria, iglesia, sinagoga o mezquita, ubicado a bordo de un barco militar o comercial, o puede ser un edificio completamente independiente. , a veces con sus propios motivos. Muchas instalaciones militares tienen capillas para uso del personal militar, normalmente bajo la dirección de un capellán militar. '
	WHERE EXISTS (SELECT * FROM COMMUNITY WHERE Type='COMMUNITY_CORE_BALANCE_BUILDINGS' AND Value= 1 );
--Granary
	UPDATE Language_es_ES
	SET Text = '+25 [ICON_FOOD] Alimentos al completar. 15% [ICON_FOOD] Alimentos es almacenado cada vez que nace un nuevo [ICON_CITIZEN] Ciudadano.[NEWLINE][NEWLINE]Permite mover  [ICON_FOOD] Alimentos de esta ciudad a través de las rutas comerciales dentro de tu Civilización.[NEWLINE][NEWLINE][ICON_RES_WHEAT] Trigo: +1 [ICON_FOOD] Alimentos.[NEWLINE][ICON_RES_BANANA] Plátanos: +1 [ICON_FOOD] Alimentos.[NEWLINE] [ICON_RES_DEER] Ciervos: +1 [ICON_FOOD] Alimentos.[NEWLINE][ICON_RES_BISON] Bisontes: +1 [ICON_FOOD] Alimentos.'
...
--Herbalist
	INSERT INTO Language_es_ES (Tag, Text)
	SELECT 'TXT_KEY_BUILDING_HERBALIST', 'Herbolario'
	WHERE EXISTS (SELECT * FROM COMMUNITY WHERE Type='COMMUNITY_CORE_BALANCE_BUILDINGS' AND Value= 1 );

	INSERT INTO Language_es_ES (Tag, Text)
	SELECT 'TXT_KEY_BUILDING_HERBALIST_STRATEGY', 'El herbolario incrementa el crecimiento de una ciudad y mejora el rendimiento de las plantaciones y casillas de bosque y selva cercanas. Constrúyelo lo más pronto posible para maximizar la bonificación, y para tomar ventaja de regiones con un gran número de características.'
	WHERE EXISTS (SELECT * FROM COMMUNITY WHERE Type='COMMUNITY_CORE_BALANCE_BUILDINGS' AND Value= 1 );

	INSERT INTO Language_es_ES (Tag, Text)
	SELECT 'TXT_KEY_BUILDING_HERBALIST_HELP', '+1 [ICON_FOOD] Alimentos cada 2 casillas de Bosque o Selva trabajadas en esta ciudad. +1 [ICON_PRODUCTION] Producción de Plantaciones y Campamentos.'
	WHERE EXISTS (SELECT * FROM COMMUNITY WHERE Type='COMMUNITY_CORE_BALANCE_BUILDINGS' AND Value= 1 );

	INSERT INTO Language_es_ES (Tag, Text)
	SELECT 'TXT_KEY_CIV5_BUILDINGS_HERBALIST_TEXT', 'La herboristería es el uso de plantas con fines medicinales y el estudio de dicho uso. Las plantas han sido la base de los tratamientos médicos a lo largo de gran parte de la historia de la humanidad y esa medicina tradicional todavía se practica ampliamente en la actualidad. La medicina moderna reconoce la herboristería como una forma de medicina alternativa, ya que la práctica de la herboristería no se basa estrictamente en la evidencia recopilada mediante el método científico. La medicina moderna, sin embargo, hace uso de muchos compuestos derivados de plantas como base para medicamentos farmacéuticos probados por evidencia, y la fitoterapia trabaja para aplicar estándares modernos de pruebas de efectividad a hierbas y medicinas que se derivan de fuentes naturales.'
	WHERE EXISTS (SELECT * FROM COMMUNITY WHERE Type='COMMUNITY_CORE_BALANCE_BUILDINGS' AND Value= 1 );
--Market
	INSERT INTO Language_es_ES (Tag, Text)
	SELECT 'TXT_KEY_BUILDING_MARKET_HELP', '[ICON_INTERNATIONAL_TRADE] Rutas Comerciales a esta ciudad generan +1 [ICON_GOLD] Oro.[NEWLINE][NEWLINE] [ICON_RES_SPICES] Canela: +1 [ICON_FOOD] Alimentos, +1 [ICON_PRODUCTION] Producción.[NEWLINE][ICON_RES_SUGAR] Azúcar: +1 [ICON_FOOD] Alimentos, +1 [ICON_GOLD] Oro.'
	WHERE EXISTS (SELECT * FROM COMMUNITY WHERE Type='COMMUNITY_CORE_BALANCE_BUILDINGS' AND Value= 1 );

	UPDATE Language_es_ES
	SET Text = 'El mercado incrementa la cantidad de [ICON_GOLD] Oro que genera una ciudad, y mejora las plantaciones cercanas de [ICON_RES_SPICES] Canela y [ICON_RES_SUGAR] Azúcar. [ICON_INTERNATIONAL_TRADE] Rutas comerciales entrantes generan +1 [ICON_GOLD] Oro para la ciudad, y +1 [ICON_GOLD] Oro para el dueño de la Ruta.'
...
--Monument
	INSERT INTO Language_es_ES (Tag, Text)
	SELECT 'TXT_KEY_BUILDING_MONUMENT_HELP', 'El costo de [ICON_CULTURE] Cultura para adquirir nuevas casillas se reduce un 25% en esta ciudad.'
	WHERE EXISTS (SELECT * FROM COMMUNITY WHERE Type='COMMUNITY_CORE_BALANCE_BUILDINGS' AND Value= 1 );
--Palace Gardens (Tradition Building)
	INSERT INTO Language_es_ES (Tag, Text)
	SELECT 'TXT_KEY_BUILDING_PALACE_GARDEN', 'Jardines del Palacio'
	WHERE EXISTS (SELECT * FROM COMMUNITY WHERE Type='COMMUNITY_CORE_BALANCE_BUILDINGS' AND Value= 1 );

	INSERT INTO Language_es_ES (Tag, Text)
	SELECT 'TXT_KEY_BUILDING_PALACE_GARDEN_HELP', 'Añade un Escritor especialista en la [ICON_CAPITAL] Capital. Contiene un espacio para [ICON_GREAT_WORK] Grandes Obras de la Literatura. Aumenta la salida de [ICON_CULTURE] Cultura de Jardines y Monumentos en +2 en todas las ciudades. Solo puede construirse a través de la política [COLOR_MAGENTA]Esplendor[ENDCOLOR].'
	WHERE EXISTS (SELECT * FROM COMMUNITY WHERE Type='COMMUNITY_CORE_BALANCE_BUILDINGS' AND Value= 1 );

	INSERT INTO Language_es_ES (Tag, Text)
	SELECT 'TXT_KEY_BUILDING_PALACE_GARDEN_PEDIA', 'Un jardín es un espacio planificado, generalmente al aire libre, reservado para la exhibición, el cultivo y el disfrute de las plantas y otras formas de la naturaleza. El jardín puede incorporar materiales tanto naturales como artificiales. La forma más común hoy en día se conoce como jardín residencial, pero el término jardín ha sido tradicionalmente más general. Los zoológicos, que exhiben animales salvajes en hábitats naturales simulados, antes se llamaban jardines zoológicos. [1][2] Los jardines occidentales se basan casi universalmente en plantas, y el jardín a menudo significa una forma abreviada de jardín botánico. '
	WHERE EXISTS (SELECT * FROM COMMUNITY WHERE Type='COMMUNITY_CORE_BALANCE_BUILDINGS' AND Value= 1 );
--Royal Astrologer (Tradition Building)
	INSERT INTO Language_es_ES (Tag, Text)
	SELECT 'TXT_KEY_BUILDING_PALACE_ASTROLOGER', 'Astrólogo Real'
	WHERE EXISTS (SELECT * FROM COMMUNITY WHERE Type='COMMUNITY_CORE_BALANCE_BUILDINGS' AND Value= 1 );

	INSERT INTO Language_es_ES (Tag, Text)
	SELECT 'TXT_KEY_BUILDING_PALACE_ASTROLOGER_HELP', 'Añade un Científico especialista en la [ICON_CAPITAL] Capital. Solo puede ser construído a través de la política [COLOR_MAGENTA]Ceremonia[ENDCOLOR].'
	WHERE EXISTS (SELECT * FROM COMMUNITY WHERE Type='COMMUNITY_CORE_BALANCE_BUILDINGS' AND Value= 1 );

	INSERT INTO Language_es_ES (Tag, Text)
	SELECT 'TXT_KEY_BUILDING_PALACE_ASTROLOGER_PEDIA', 'La astrología, en su sentido más amplio, es la búsqueda de significado en el cielo. La evidencia temprana de que los humanos intentaron conscientemente medir, registrar y predecir los cambios estacionales por referencia a los ciclos astronómicos, aparece como marcas en los huesos y las paredes de las cuevas, que muestran que los ciclos lunares se estaban notando ya hace 25.000 años. Este fue un primer paso hacia el registro de la influencia de la Luna sobre las mareas y los ríos, y hacia la organización de un calendario comunal. Los agricultores abordaron las necesidades agrícolas con un conocimiento cada vez mayor de las constelaciones que aparecen en las diferentes estaciones y utilizaron el surgimiento de grupos de estrellas particulares para anunciar inundaciones anuales o actividades estacionales. Para el tercer milenio a. C., las civilizaciones tenían una conciencia sofisticada de los ciclos celestes y pueden haber orientado los templos en alineación con los ascensos helíacos de las estrellas.'
	WHERE EXISTS (SELECT * FROM COMMUNITY WHERE Type='COMMUNITY_CORE_BALANCE_BUILDINGS' AND Value= 1 );
--Royal Guardhouse (Tradition Building)
	INSERT INTO Language_es_ES (Tag, Text)
	SELECT 'TXT_KEY_BUILDING_CAPITAL_ENGINEER', 'Guardia Real'
	WHERE EXISTS (SELECT * FROM COMMUNITY WHERE Type='COMMUNITY_CORE_BALANCE_BUILDINGS' AND Value= 1 );

	INSERT INTO Language_es_ES (Tag, Text)
	SELECT 'TXT_KEY_BUILDING_CAPITAL_ENGINEER_HELP', 'Añade un Ingeniero especialista en la [ICON_CAPITAL] Capital. Solo puede construirse a través de la política [COLOR_MAGENTA]Justicia[ENDCOLOR].'
	WHERE EXISTS (SELECT * FROM COMMUNITY WHERE Type='COMMUNITY_CORE_BALANCE_BUILDINGS' AND Value= 1 );

	INSERT INTO Language_es_ES (Tag, Text)
	SELECT 'TXT_KEY_BUILDING_CAPITAL_ENGINEER_PEDIA', 'Una Guardia Real describe a cualquier grupo de guardaespaldas militares, soldados o sirvientes armados responsables de la protección de una persona real, como Emperador/Emperatriz, Rey/Reina o Príncipe/Princesa. A menudo son una unidad de élite de las fuerzas armadas regulares, o están designados como tales, y pueden mantener derechos o privilegios especiales.'
	WHERE EXISTS (SELECT * FROM COMMUNITY WHERE Type='COMMUNITY_CORE_BALANCE_BUILDINGS' AND Value= 1 );
--Shrine (no need changes)
--State Treasury (Tradition Building)
	INSERT INTO Language_es_ES (Tag, Text)
	SELECT 'TXT_KEY_BUILDING_PALACE_TREASURY', 'Tesorería Estatal'
	WHERE EXISTS (SELECT * FROM COMMUNITY WHERE Type='COMMUNITY_CORE_BALANCE_BUILDINGS' AND Value= 1 );

	INSERT INTO Language_es_ES (Tag, Text)
	SELECT 'TXT_KEY_BUILDING_PALACE_TREASURY_HELP', '+25% a la tasa de generación de [ICON_GREAT_PEOPLE] Grandes Personajes en esta ciudad. Añade un Mercader especialista en la [ICON_CAPITAL] Capital. Solo puede construirse a través de la política [COLOR_MAGENTA]Majestad[ENDCOLOR].'
	WHERE EXISTS (SELECT * FROM COMMUNITY WHERE Type='COMMUNITY_CORE_BALANCE_BUILDINGS' AND Value= 1 );

	INSERT INTO Language_es_ES (Tag, Text)
	SELECT 'TXT_KEY_BUILDING_PALACE_TREASURY_PEDIA', 'El término tesorería se utilizó por primera vez en la época clásica para describir los edificios votivos erigidos para albergar regalos a los dioses, como el Tesoro de Siphnian en Delfos o muchos edificios similares erigidos en Olimpia, Grecia por ciudades-estado en competencia para impresionar a otros durante los antiguos Juegos Olímpicos. Juegos. En la Antigua Grecia, las tesorerías casi siempre estaban físicamente incorporadas dentro de edificios religiosos como templos, haciendo así sacrosantos los fondos estatales y agregando restricciones morales a las penales para aquellos que tuvieran acceso a estos fondos.'
	WHERE EXISTS (SELECT * FROM COMMUNITY WHERE Type='COMMUNITY_CORE_BALANCE_BUILDINGS' AND Value= 1 );
--Stone Works
	UPDATE Language_es_ES
	SET Text = 'Permite mover [ICON_PRODUCTION] Producción de esta ciudad a través de Rutas Comerciales dentro de tu civilización.[NEWLINE][NEWLINE][ICON_RES_MARBLE] Mármol: +1 [ICON_PRODUCTION] Producción, +1 [ICON_GOLD] Oro.[NEWLINE][ICON_RES_STONE] Piedra: +2 [ICON_PRODUCTION] Producción.[NEWLINE][ICON_RES_SALT] Sal: +2 [ICON_GOLD] Oro.[NEWLINE][NEWLINE]La ciudad debe tener al menos uno de estos recursos mejorado.'
	...
	UPDATE Language_es_ES
	SET Text = 'El taller de Cantería solo puede construirse en ciudades que hayan mejorado alguno de estos recursos: [ICON_RES_STONE] Piedra, [ICON_RES_SALT] Sal, [ICON_RES_MARBLE] Mármol, o [ICON_RES_URANIUM] Uranio. El taller de cantería incrementa la [ICON_PRODUCTION] Producción y permite mover  [ICON_PRODUCTION] desde esta ciudad a través de rutas comerciales internas.'
...
--Throne Room (Tradition Building)
	INSERT INTO Language_es_ES (Tag, Text)
	SELECT 'TXT_KEY_BUILDING_THRONE_ROOM', 'Sala del Trono'
	WHERE EXISTS (SELECT * FROM COMMUNITY WHERE Type='COMMUNITY_CORE_BALANCE_BUILDINGS' AND Value= 1 );

	INSERT INTO Language_es_ES (Tag, Text)
	SELECT 'TXT_KEY_BUILDING_CAPITAL_THRONE_ROOM_HELP', '+25% a la duración de las [ICON_GOLDEN_AGE] Edades de Oro, +10% a todos los rendimientos de la Ciudad. Incrementa el Suministro de Unidades Militares en 4. 2 especialistas en la ciudad no producirán [ICON_HAPPINESS_3] Infelicidad por Urbanización.[NEWLINE][NEWLINE]Añade un Músico especialista en la [ICON_CAPITAL] Capital. Contiene un espacio para [ICON_GREAT_WORK] Grandes Obras de la Música. Solo puede construirse a través de la política [COLOR_MAGENTA]Tradición[ENDCOLOR].'
	WHERE EXISTS (SELECT * FROM COMMUNITY WHERE Type='COMMUNITY_CORE_BALANCE_BUILDINGS' AND Value= 1 );

	INSERT INTO Language_es_ES (Tag, Text)
	SELECT 'TXT_KEY_BUILDING_CAPITAL_THRONE_ROOM_PEDIA', 'Una sala del trono es la sala, a menudo más bien una sala, en la residencia oficial de la corona, ya sea un palacio o un castillo fortificado, donde el trono de una figura de alto nivel (generalmente un monarca) se instala con elaborada pompa, generalmente elevado a menudo con escalones y bajo un dosel, los cuales son parte de la noción original de la palabra griega thronos.'
	WHERE EXISTS (SELECT * FROM COMMUNITY WHERE Type='COMMUNITY_CORE_BALANCE_BUILDINGS' AND Value= 1 );
--Walls
	UPDATE Language_es_ES
	SET Text = 'Las murallas incrementan la fuerza de defensa y puntos de golpe de una ciudad, haciendo que sea más dificil de capturar. Incrementa el suministro de unidades militares en un 10% de la población de la ciudad, e incrementa el ataque a distancia en 1. También ayuda a gestionar el modificador de las Necesidades del Imperio. Las murallas son muy útiles en ciudades ubicadas a lo largo de la frontera.'
	...
	Tag, Text)
	SELECT 'TXT_KEY_BUILDING_WALLS_HELP', 'Incrementa el [ICON_RANGE_STRENGTH] rango de golpe a distancia de la ciudad en 1. La cantidad de Unidades suministradas por la población de esta ciudad se incrementa en 10%.[NEWLINE][NEWLINE]Reduce un 5% el modificador de [ICON_HAPPINESS_3] Necesidades del Imperio en esta ciudad.'
	WHERE EXISTS (SELECT * FROM COMMUNITY WHERE Type='COMMUNITY_CORE_BALANCE_BUILDINGS' AND Value= 1 );
--Well
	INSERT INTO Language_es_ES (
	Tag, Text)
	SELECT 'TXT_KEY_BUILDING_WELL', 'Pozo'
	WHERE EXISTS (SELECT * FROM COMMUNITY WHERE Type='COMMUNITY_CORE_BALANCE_BUILDINGS' AND Value= 1 );

	INSERT INTO Language_es_ES (
	Tag, Text)
	SELECT 'TXT_KEY_BUILDING_WELL_STRATEGY', 'El Pozo solo se puede construir en una ciudad sin Río. Incrementa la Producción de la ciudad basado en su Población, aunque es menos eficiente que el Molino de Agua.'
	WHERE EXISTS (SELECT * FROM COMMUNITY WHERE Type='COMMUNITY_CORE_BALANCE_BUILDINGS' AND Value= 1 );

	INSERT INTO Language_es_ES (
	Tag, Text)
	SELECT 'TXT_KEY_BUILDING_WELL_HELP', '+1 [ICON_PRODUCTION] Producción por cada 5 [ICON_CITIZEN] Ciudadanos en la Ciudad.[NEWLINE][NEWLINE]La ciudad no debe construirse junto a un Río.'
	WHERE EXISTS (SELECT * FROM COMMUNITY WHERE Type='COMMUNITY_CORE_BALANCE_BUILDINGS' AND Value= 1 );

	INSERT INTO Language_es_ES (
	Tag, Text)
	SELECT 'TXT_KEY_CIV5_BUILDINGS_WELL_TEXT', 'Los pozos revestidos de madera se conocen de la Cultura de la Alfarería Lineal del Neolítico temprano, por ejemplo, en Kückhoven (un centro periférico de Erkelenz), con fecha de 5090 a. C. y Eythra, con fecha de 5200 a. C. en Schletz (un centro periférico de Asparn an der Zaya) en Austria.  Los aborígenes australianos dependían de los pozos para sobrevivir en el duro desierto australiano. Cavarían, sacarían arena y lodo para alcanzar agua limpia, luego cubrirían la fuente con spinifex para evitar el deterioro. Los no aborígenes llaman a estos pozos nativos, baños o remojos.  Los pozos escalonados son comunes en el oeste de la India. En estos pozos, se puede llegar al agua bajando una serie de escalones. Pueden estar cubiertos y, a menudo, son de importancia arquitectónica. Muchos pozos escalonados también se utilizaron para el ocio, proporcionando alivio del calor diurno. Un qanat es un antiguo sistema de recolección de agua formado por una serie de pozos y canales de agua subterráneos conectados que recolecta el agua que fluye de una fuente generalmente a una distancia, la almacena y luego lleva el agua a la superficie usando la gravedad. Gran parte de la población de Irán y otros países áridos de Asia y África del Norte dependía históricamente del agua de los qanats; las áreas de población correspondían estrechamente a las áreas donde son posibles los qanats. En Egipto, se utilizan shadoofs y sakiehs. Sin embargo, cuando se comparan entre sí, el Sakkieh es mucho más eficiente, ya que puede traer agua desde una profundidad de 10 metros (frente a los 3 metros de la sombra). El Sakieh es la versión egipcia del Noria. A partir de la Edad del Hierro, los pozos son elementos arqueológicos habituales, tanto con fustes de madera como con revestimientos de cestería. Los pozos más antiguos conocidos del mundo, ubicados en Chipre, datan del 7500 a. C. En Israel se han descubierto dos pozos del período neolítico, alrededor del 6500 a. C. Uno está en Atlit, en la costa norte de Israel, y el otro es el valle de Jezreel.'
	WHERE EXISTS (SELECT * FROM COMMUNITY WHERE Type='COMMUNITY_CORE_BALANCE_BUILDINGS' AND Value= 1 );
/**************************************************************
...
--Palace 
	UPDATE Language_es_ES
	SET Text = '+1 [ICON_RESEARCH] Ciencia por cada 3 [ICON_CITIZEN] Ciudadanos en la ciudad. Contiene un espacio para una [ICON_GREAT_WORK] Gran Obra de Arte. 1 especialista en esta ciudad no producirá [ICON_HAPPINESS_3] Infelicidad por Urbanización.[NEWLINE][NEWLINE]Cuando uno de los siguientes [COLOR_POSITIVE_TEXT]Eventos Históricos[ENDCOLOR] es completado, recibe un aumento de [ICON_TOURISM] Turismo con todas las civilizaciones conocidas basado en la [ICON_CULTURE] Cultura reciente y la salida de [ICON_TOURISM] Turismo:[NEWLINE][ICON_BULLET] Nace un [ICON_GREAT_PEOPLE] Gran Personaje[NEWLINE][ICON_BULLET] Construye una  [ICON_GOLDEN_AGE] Maravilla del Mundo[NEWLINE][ICON_BULLET] Gana una [ICON_WAR] Guerra (Puntuación de Guerra 25+)[NEWLINE][ICON_BULLET] ENtra en una nueva [ICON_RESEARCH] Era[NEWLINE][NEWLINE][ICON_CONNECTED] Conectar otras ciudades con la [ICON_CAPITAL] Capital mediante caminos producirá [ICON_GOLD] Oro adicional.'
...
--Halicarnassus
	UPDATE Language_es_ES
	SET Text = 'Proporciona un Taller de Cantería [COLOR_POSITIVE_TEXT]Gratis[ENDCOLOR] en la ciudad que se construye. Comienza un "Día de Amamos al Rey". Durante el "Día de Amamos al Rey", se incrementa la [ICON_PRODUCTION] Producción, [ICON_RESEARCH] Ciencia, y [ICON_GOLD] Oro al 10%.'
...
--Petra
	UPDATE Language_es_ES
	SET Text = 'Solo se puede construir en una ciudad en o junto al Desierto. Concede 1 espacio de [ICON_INTERNATIONAL_TRADE] Rutas Comerciales [COLOR_POSITIVE_TEXT]Adicional[ENDCOLOR] y aparece una Caravana [COLOR_POSITIVE_TEXT]Gratis[ENDCOLOR] en la ciudad que se construye. +1 [ICON_GOLD] Oro en todas las casillas de desierto trabajadas por esta ciudad. +6 [ICON_CULTURE] Cultura cuando se descubra la [COLOR_CYAN]Arquelogía[ENDCOLOR].'
...
-- Pyramids 
	UPDATE Language_es_ES
	SET Text = 'Aparece un Colono cerca de la ciudad. +50 puntos de [ICON_GOLDEN_AGE] Edad de Oro cuando se gaste un [ICON_GREAT_PEOPLE] Gran Personaje, escala con la era.'
...
--Zeus
	UPDATE Language_es_ES
	SET Text = 'Porporciona Cuarteles [COLOR_POSITIVE_TEXT]Gratis[ENDCOLOR] en la ciudad que se construye. Todas las unidades entrenadas en esta ciudad ganan +15% [ICON_STRENGTH] Fuerza de Combate al atacar [COLOR_POSITIVE_TEXT]Ciudades[ENDCOLOR]. -1 [ICON_HAPPINESS_3] Infelicidad por [ICON_FOOD]/[ICON_PRODUCTION] Angustia en esta ciudad.'
...
--Stonehenge
	INSERT INTO Language_es_ES (Tag, Text)
	SELECT 'TXT_KEY_BUILDING_STONEHENGE_HELP', 'Proporciona un Consejo [COLOR_POSITIVE_TEXT]Gratis[ENDCOLOR] y 50 [ICON_PEACE] Fé en la ciudad que se construye.'
	WHERE EXISTS (SELECT * FROM COMMUNITY WHERE Type='COMMUNITY_CORE_BALANCE_BUILDINGS' AND Value= 1 );
--Artemisa
	UPDATE Language_es_ES
	SET Text = 'Proporciona un Herbolario [COLOR_POSITIVE_TEXT]Gratis[ENDCOLOR] en la ciudad que se construye. +10% [ICON_FOOD] Alimentos en todas las ciudades. +25% [ICON_PRODUCTION] Producción cuando se construyen unidades de Combate a Distancia. 2 especialistas en esta ciudad no producen [ICON_HAPPINESS_3] Infelicidad por Urbanización.'
...
--Amphitheater
	INSERT INTO Language_es_ES (Tag, Text)
	SELECT 'TXT_KEY_BUILDING_AMPHITHEATER_HELP', '+33% a la tasa de generación de [ICON_GREAT_WRITER] Grandes Escritores en esta ciudad, y todos los Gremios de Escritores producen +1 [ICON_GOLD] Oro.[NEWLINE][NEWLINE][ICON_RES_DYE] Tintes: +1 [ICON_CULTURE] Cultura, +1 [ICON_GOLD] Oro.[NEWLINE][ICON_RES_SILK] Seda: +1 [ICON_CULTURE] Cultura, +1 [ICON_GOLD] Oro.[NEWLINE][ICON_RES_LAPIS] Lapislázuli: +2 [ICON_CULTURE] Cultura.[NEWLINE][NEWLINE]+2 [ICON_CULTURE] Cultura al conseguir bonif. por tema.'
	WHERE EXISTS (SELECT * FROM COMMUNITY WHERE Type='COMMUNITY_CORE_BALANCE_BUILDINGS' AND Value= 1 );
		
	UPDATE Language_es_ES
	SET Text = 'El anfitearo incrementa la [ICON_CULTURE] Cultura de una ciudad y concede bonificaciones a las plantaciones cercanas de [ICON_RES_DYE] Tintes y [ICON_RES_SILK] Seda. También aumenta la tasa de generación de Grandes Escritores en la ciudad y el valor de todos los Gremios de Escritores. Contiene 2 espacios para Grandes Obras de la Literatura'
	...
	INSERT INTO Language_es_ES (Tag, Text)
	SELECT 'TXT_KEY_THEMING_BONUS_AMPHITHEATER_NATIONAL', 'Epopeyas Nacionales'
	WHERE EXISTS (SELECT * FROM COMMUNITY WHERE Type='COMMUNITY_CORE_BALANCE_BUILDINGS' AND Value= 1 );

	INSERT INTO Language_es_ES (Tag, Text)
	SELECT 'TXT_KEY_THEMING_BONUS_AMPHITHEATER_GLOBAL', 'Epopeyas Globales'
	WHERE EXISTS (SELECT * FROM COMMUNITY WHERE Type='COMMUNITY_CORE_BALANCE_BUILDINGS' AND Value= 1 );

	INSERT INTO Language_es_ES (Tag, Text)
	SELECT 'TXT_KEY_AMPHITHEATER_THEMING_BONUS_HELP', 'Para maximizar su bonificación, asegúrese de que los espacios de Grandes Obras estén llenos de Escritos de su propia Civilización o de Civilizaciones distintas a la suya. '
	WHERE EXISTS (SELECT * FROM COMMUNITY WHERE Type='COMMUNITY_CORE_BALANCE_BUILDINGS' AND Value= 1 );
--Aqueduct
	UPDATE Language_es_ES
	SET Text = 'Almacena el 15% de los [ICON_FOOD] Alimentos cada vez que nace un nuevo [ICON_CITIZEN] Ciudadano, y +25% de la [ICON_PRODUCTION] Producción de una ciudad es agregada a la [ICON_PRODUCTION] Producción actual. +2 [ICON_FOOD] Alimentos en Lagos y Oasis trabajados por esta ciudad.[NEWLINE][NEWLINE]-1 [ICON_HAPPINESS_3] Infelicidad por [ICON_GOLD] Pobreza.'
	...
	UPDATE Language_es_ES
	SET Text = 'El Acueducto reduce la cantidad de [ICON_FOOD] Alimentos que una ciudad necesita para crecer en tamaño (ciudadanos) al 15%. Construye acueductos en ciudades que quieras que crezcan con el paso del tiempo.'
...
--Arena
	INSERT INTO Language_es_ES (Tag, Text)
	SELECT 'TXT_KEY_BUILDING_COLOSSEUM_HELP', '+2 [ICON_TOURISM] Turismo. Cuarteles, Forjas, y Armerías en esta ciudad ganan +2 [ICON_PRODUCTION] Producción. -1 [ICON_HAPPINESS_3] Infelicidad por [ICON_CULTURE] Aburrimiento.'
	WHERE EXISTS (SELECT * FROM COMMUNITY WHERE Type='COMMUNITY_CORE_BALANCE_BUILDINGS' AND Value= 1 );

	UPDATE Language_es_ES
	SET Text = 'La Arena genera Turismo, reduce el Aburrimiento en una ciudad y otorga Cultura adicional. Constrúyalos para combatir la infelicidad por aburrimiento, aumentar su cultura y mejorar la producción de sus edificios militares. '
	...
	UPDATE Language_es_ES
	SET Text = 'Una arena es un área cerrada, a menudo circular u ovalada, diseñada para exhibir obras de teatro, actuaciones musicales o eventos deportivos. La palabra deriva del latín harena, una arena particularmente fina/suave que se usa para absorber sangre en arenas antiguas como el Coliseo de Roma. Se compone de un gran espacio abierto rodeado en la mayoría o en todos los lados por asientos escalonados para los espectadores. La característica clave de una arena es que el espacio para eventos es el punto más bajo, lo que permite la máxima visibilidad. Las arenas suelen estar diseñadas para acomodar a un gran número de espectadores. '
...
--Watermill
UPDATE Language_es_ES
SET Text = '+1 [ICON_PRODUCTION] Producción por cada 4 [ICON_CITIZEN] Ciudadanos en la ciudad.[NEWLINE][NEWLINE]Can only be built in a City next to a River.'
...
UPDATE Language_es_ES
SET Text = 'El Molino de Agua puede construirse únicamente en una ciudad ubicada al lado de un río. El Molino de Agua incrementa la [ICON_PRODUCTION] Producción de una ciudad más eficientemente que el Pozo (por cantidad de ciudadanos en una ciudad).'
...
--Garden
UPDATE Language_es_ES
SET Text = '+25% a la generación de [ICON_GREAT_PEOPLE] Grandes Personajes en esta ciudad.[NEWLINE][NEWLINE]1 especialista en esta ciudad ya no producirá [ICON_HAPPINESS_3] Infelicidad por Urbanización.[NEWLINE][NEWLINE]Oasis: +2 [ICON_GOLD] Oro.[NEWLINE][ICON_RES_CITRUS] Cítricos: +1 [ICON_FOOD] Alimentos, +1 [ICON_GOLD] Oro.[NEWLINE][ICON_RES_COCOA] Cacao: +1 [ICON_FOOD] Alimentos, +1 [ICON_GOLD] Oro.'
...
UPDATE Language_es_ES
SET Text = 'El Jardín incrementa la velocidad a la que se generan los [ICON_GREAT_PEOPLE] Grandes Personajes en la ciudad en un 25%, también aumenta los alimentos en Oasis. El Jardín requiere un Acueducto en la ciudad para poder construirlo.'
...
--Medical Lab
UPDATE Language_es_ES
SET Text = '+2 [ICON_CITIZEN] Ciudadanos al completar. Se almacena el 15% de [ICON_FOOD] Alimentos cada vez que nace un nuevo [ICON_CITIZEN] Ciudadano. Científicos, Mercaderes e Ingenieros especialistas en la ciudad producen +2 de su rendimiento base.[NEWLINE][NEWLINE]-1 [ICON_HAPPINESS_3] Infelicidad por [ICON_GOLD] Pobreza.'
...
UPDATE Language_es_ES
SET Text = 'El Laboratorio Médico es un edificio del final del juego que reduce la cantidad de [ICON_FOOD] Alimentos que una ciudad necesita para crecer en tamaño (ciudadanos) en un 15% y aumenta la producción de Ciencia de los especialistas en la ciudad. Es necesario haber construído un Hospital en la ciudad previmante para construir el Laboratorio Médico.'
...
UPDATE Language_es_ES
SET Text = 'Proporciona espías adicionales basado en el número de Ciudades Estado en el juego y asciende todos los espías existentes. También reduce un 15% la efectividad de los espías enemigos. Debes tener una Estación de Policía en todas las ciudades.'
WHERE Tag = 'TXT_KEY_BUILDING_INTELLIGENCE_AGENCY_HELP';

UPDATE Language_es_ES
SET Text = 'Una importante Maravilla Nacional para civilizaciónes avanzadas. La Agencia Nacional de Inteligencia proporciona espías adicionales, asciende todos los espías existentes y concede una reducción del 15% en la efectividad de espías enemigos. La civilización debe tener una Estación de Policía en todas las ciudades para poder construir la Agencia Nacional de Inteligencia'
...
-- Monastery
	UPDATE Language_es_ES
	SET Text = 'Edificios religiosos comprados con [ICON_PEACE] Fé. Incrementan los [ICON_FOOD] Alimentos, [ICON_RESEARCH] Ciencia y [ICON_PEACE] Fé de las ciudades. Requiere la adopción de la política Social Fidelidad.'
